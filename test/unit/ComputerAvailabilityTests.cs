...
namespace CommonLibTest
{
    public class ComputerAvailabilityTests : IDisposable
    {
        ...
        public ComputerAvailabilityTests(ITestOutputHelper testOutputHelper)
        {
            ...
            m.Setup(x => x.CheckPort(It.IsAny<string>(), It.IsAny<int>(), It.IsAny<int>())).Returns(Task.FromResult(false));
            _falsePortScanner = m.Object;
            
            var m2 = new Mock<PortScanner>();
            m2.Setup(x => x.CheckPort(It.IsAny<string>(), It.IsAny<int>(), It.IsAny<int>())).Returns(Task.FromResult(true));
