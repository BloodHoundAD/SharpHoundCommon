...
namespace SharpHoundCommonLib.Processors
{
    public class LSAServer : IDisposable
    {
        ...
        public LSAServer(string computerName, ILDAPUtils utils = null, NativeMethods methods = null)
        {
            Logging.Trace($"Opening LSA server for {computerName}");
            ...
            Logging.Trace($"LSAOpenPolicy returned {status} for {computerName}");
        ...
        public IEnumerable<string> ReadLSAPrivilege(string privilege)
        {
            ...
            Logging.Trace($"LSAEnumerateAccountsWithUserRight returned {status} for {privilege} on {_computerName}");
            if (status != NativeMethods.NtStatus.StatusSuccess)
            {
            ...
            Logging.Trace($"LSAEnumerateAccountsWithUserRight returned {count} objects for privilege {privilege} on {_computerName}");
